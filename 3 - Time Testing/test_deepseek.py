...
def test_deepseek(command):
    """Test a command with DeepSeek and return elapsed time. Fresh client each call."""
...
def main():
    print("=" * 60)
    print("DeepSeek R1 Shell Command Timing Test")
    ...
    if all_times:
        ...
        results = ["DeepSeek R1: No successful tests"]
    
    with open("deepseek_times.txt", "w") as f:
        for line in results:
            f.write(line + "\n")
    
    print("\n" + "=" * 60)
    print("Results saved to deepseek_times.txt")
    print("=" * 60)
    for line in results:
        print(line)
