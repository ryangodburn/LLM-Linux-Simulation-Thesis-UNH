...
def test_llama(command):
    """Test a command with Llama and return elapsed time. Fresh client each call."""
    from openai import OpenAI
    client = OpenAI(base_url="http://localhost:11434/v1", api_key="none")
    messages = copy.deepcopy(INITIAL_MESSAGES)
    messages.append({"role": "user", "content": command})
    start_time = time.time()
    client.chat.completions.create(model="llama4:maverick", messages=messages)
    return time.time() - start_time


def main():
    print("=" * 60)
    print("Llama 4 Maverick Shell Command Timing Test")
    ...
    if all_times:
        ...
        results = ["Llama 4 Maverick: No successful tests"]
    
    with open("llama_times.txt", "w") as f:
        for line in results:
            f.write(line + "\n")
    
    print("\n" + "=" * 60)
    print("Results saved to llama_times.txt")
    print("=" * 60)
    for line in results:
        print(line)
